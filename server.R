...
# 🔥 NEW: Variable labels for heat maps
var_labels <- list(
  FunctionalAssessment = "Functional Score", ADL = "ADL Score", 
  MemoryComplaints = "Memory Complaints", MMSE = "MMSE Score", 
  BehavioralProblems = "Behavioral Problems", EducationLevel = "Education Level",
  SerumCreatinine = "Serum Creatinine", GFR = "GFR", Itching = "Itching", 
  FastingBloodSugar = "Fasting Blood Sugar", MuscleCramps = "Muscle Cramps",
  BUNLevels = "BUN Levels", ProteinInUrine = "Protein in Urine", 
  SystolicBP = "Systolic BP", HbA1c = "HbA1c", BMI = "BMI",
  UPDRS = "UPDRS Score", Tremor = "Tremor", Rigidity = "Rigidity", 
  Bradykinesia = "Bradykinesia", MoCA = "MoCA Score", 
  PosturalInstability = "Postural Instability", Age = "Age", 
  Depression = "Depression", Diabetes = "Diabetes",
  FrequentUrination = "Frequent Urination", Hypertension = "Hypertension", 
  ExcessiveThirst = "Excessive Thirst", UnexplainedWeightLoss = "Weight Loss", 
  Smoking = "Smoking"
)
...
# 🔥 NEW: HEAT MAP GENERATOR
# 🔥 NEW: HEAT MAP GENERATOR
# 🔥 NEW: HEAT MAP GENERATOR
# 🔥 HEAT MAP GENERATOR
make_heatmap <- function(model, user_data, dataset, var_labels) {
  coefs <- coef(model)[-1]
  predictor_names <- names(coefs)
  dataset_means <- sapply(predictor_names, function(v) mean(dataset[[v]], na.rm = TRUE))
  ...
  direction <- ifelse(raw_contributions > 0, "Increases", "Decreases")
  
  # 🔧 FIX: Handle missing labels
  variable_labels <- sapply(predictor_names, function(x) {
    if (!is.null(var_labels[[x]])) {
      var_labels[[x]]
    } else {
      x  # Use the variable name itself if no label exists
    }
  })
  ...
  # Color matrix
  n <- nrow(contrib_df)
  color_matrix <- matrix(contrib_df$Contribution, nrow = n, ncol = 1)
  color_palette_red <- colorRampPalette(c("white", "#e74c3c"))(100)
  color_palette_blue <- colorRampPalette(c("white", "#3498db"))(100)
  
  contrib_colors <- sapply(1:n, function(i) {
    intensity <- min(round(contrib_df$Contribution[i]), 100)
    if (contrib_df$Direction[i] == "Increases") {
      color_palette_red[max(1, intensity)]
    } else {
      color_palette_blue[max(1, intensity)]
    }
  })
  
  # Plot
  par(mar = c(3, 12, 3, 6))
  image(t(color_matrix), col = contrib_colors, axes = FALSE, xlab = "", ylab = "",
        main = "Risk Factor Contribution Heat Map")
  axis(2, at = seq(0, 1, length.out = n), labels = contrib_df$Variable, las = 1, cex.axis = 0.75)
  axis(4, at = seq(0, 1, length.out = n), labels = paste0(contrib_df$Contribution, "%"), 
       las = 1, cex.axis = 0.75, tick = FALSE)
  legend("bottom", legend = c("Increases Risk", "Decreases Risk"),
         fill = c("#e74c3c", "#3498db"), horiz = TRUE, cex = 0.7, bty = "n", xpd = TRUE, inset = c(0, -0.15))
}

# 🔥 MODIFIED: Added heatmap_id parameter
result_card <- function(prob, disease, heatmap_id) {
  ...
  div(
    ...
    tags$div(class = "progress", style = "height:22px; border-radius:6px; margin-bottom:12px;",
             tags$div(class = paste0("progress-bar bg-", bar_color), role = "progressbar",
    ...
    tags$div(
      style = paste0("padding:12px; border-radius:6px; font-size:13px; background:", bg_color,
                     "; color:", txt_color, "; border-left:4px solid ", hex_color, ";"),
      tags$b(paste0(level, " Risk — ")), msg
    ),
    # 🔥 NEW: Heat map section
    tags$div(style = "margin-top:20px;",
             h5("🔥 What's Driving Your Risk?", style = "color:#444; margin-bottom:10px;"),
             plotOutput(heatmap_id, height = "300px")
    )
  )
}

waiting_card <- function() {
  div(style = "background:#f0f4f8; border-radius:8px; padding:30px; text-align:center; color:#888; margin-top:10px;",
      tags$h3("📊", style = "font-size:48px; margin-bottom:8px;"),
      p("Fill in your profile and click ", tags$b("Calculate Risk"), " to see your estimated probability.")
  )
}

make_boxplot <- function(dataset, col, diagnosis_col, user_val, xlabel, no_label = "No", yes_label = "Yes") {
  plot_data <- dataset
  ...
  par(mar = c(5, 4, 3, 1))
  boxplot(plot_data[[col]] ~ plot_data$DiagnosisLabel, col = c("#5b9bd5", "#e74c3c"), 
          main = xlabel, xlab = "", ylab = xlabel, outline = FALSE, cex.main = 0.95, 
          cex.axis = 0.85, border = c("#2176ae", "#c0392b"))
  abline(h = user_val, col = "#c0392b", lwd = 2, lty = 2)
  legend("topright", legend = "Your value", col = "#c0392b", lwd = 2, lty = 2, cex = 0.75, bty = "n")
}

make_barchart <- function(dataset, col, diagnosis_col, user_val, xlabel, disease_label = "Disease") {
  ...
  par(mar = c(5, 4, 4, 1))
  bp <- barplot(c(pct_no, pct_yes), names.arg = c(paste("No", xlabel), paste("Yes", xlabel)),
                col = c("#5b9bd5", "#e74c3c"), border = c("#2176ae", "#c0392b"), main = xlabel,
                ylab = paste0("% with ", disease_label), ylim = c(0, 115),
                cex.main = 0.95, cex.axis = 0.85, cex.names = 0.8)
  
  arrows(bp, c(ci_no[1], ci_yes[1]), bp, c(ci_no[2], ci_yes[2]), 
         angle = 90, code = 3, length = 0.08, lwd = 1.5, col = "#333")
  
  label_y <- c(ci_no[2], ci_yes[2]) + 6
  rect(bp - 0.4, label_y - 3, bp + 0.4, label_y + 3, col = "white", border = NA)
  text(bp, label_y, labels = paste0(round(c(pct_no, pct_yes), 1), "%"), cex = 0.8, col = "#222", font = 2)
  
  user_bar <- if (user_val == 1) 2 else 1
  rect(bp[user_bar] - 0.5, 0, bp[user_bar] + 0.5, c(pct_no, pct_yes)[user_bar],
       border = "#f0a500", lwd = 3, col = NA)
  
  legend("topright", legend = c("Your group", "95% CI"), col = c("#f0a500", "#333"), 
         lwd = c(3, 1.5), cex = 0.75, bty = "n")
}
...
render_disease_plots <- function(prefix, cfg, input, output) {
  ...
  make_pairs <- function(ids) {
    if (length(ids) == 0) return(list())  # 🔧 FIX: Handle empty vectors
  ...
  dist_ui <- tags$div(
    ...
    tags$button(class = "btn btn-outline-secondary btn-sm", `data-toggle` = "collapse",
                `data-target` = paste0("#", cfg$collapse_id), "📊 Show / Hide Variable Distributions"),
    tags$div(id = cfg$collapse_id, class = "collapse",
             tags$div(style = "margin-top:12px; background:#fff; border-radius:8px; padding:16px; box-shadow:0 2px 8px rgba(0,0,0,0.08);",
                      ...
                      glm_rows, divider_ui, context_rows
             )
    )
  )
  ...
  for (i in seq_along(cfg$continuous)) {
    local({
      ...
      output[[id]] <- renderPlot({
        make_boxplot(ds, v$col, cfg$diagnosis_col, eval(v$input, list(input = input)), 
                     v$label, no_label = cfg$no_label, yes_label = cfg$yes_label)
      })
    })
  }
  ...
  for (i in seq_along(cfg$binary)) {
    local({
      ...
      output[[id]] <- renderPlot({
        make_barchart(ds, v$col, cfg$diagnosis_col, as.numeric(eval(v$input, list(input = input))), 
                      v$label, disease_label = cfg$disease_label)
      })
    })
  }
...
quiz_questions <- list(
  list(
    ...
    options = c("MoCA (Montreal Cognitive Assessment)", "MMSE (Mini-Mental State Examination)", 
                "ACE-R (Addenbrooke's Cognitive Examination)", "CDR (Clinical Dementia Rating)"),
    correct = 2,
  ...
  list(
    ...
    options = c("Functional Assessment Score", "Activities of Daily Living (ADL) Score", "Both A and B", "Neither - we don't measure this"),
    correct = 3,
  ...
  list(
    ...
    options = c("Glucose Filtration Rate - measures blood sugar", "Glomerular Filtration Rate - measures how well kidneys filter waste", 
                "General Function Rating - overall kidney health score", "Glucose-Free Range - safe blood sugar levels"),
    correct = 2,
  ...
  list(
    ...
    options = c("Serum creatinine", "BUN (Blood Urea Nitrogen) levels", "Both A and B", "HbA1c"),
    correct = 3,
  ...
  list(
    ...
    options = c("Tremor", "Memory complaints", "Itching severity", "Excessive thirst"),
    correct = 3,
  ...
  list(
    ...
    options = c("Brain imaging results", "Unified Parkinson's Disease Rating Scale - motor and non-motor symptoms", 
                "Ultrasound Parkinson's Detection Rate", "Urban Pollution Disease Risk Score"),
    correct = 2,
  ...
  list(
    ...
    options = c("Tremor, Rigidity, Bradykinesia", "Tremor, Memory Loss, Confusion", 
                "Rigidity, Itching, Muscle Cramps", "Bradykinesia, ADL, MMSE"),
    correct = 1,
  ...
  list(
    ...
    options = c("MMSE", "MoCA (Montreal Cognitive Assessment)", "Functional Assessment", "ADL Score"),
    correct = 2,
  ...
  list(
    ...
    options = c("Current blood sugar - it only affects diabetes", "Average blood sugar over 2-3 months - both diseases are linked", 
                "Hemoglobin count - unrelated to either disease", "Hormone levels - affects metabolism only"),
    correct = 2,
  ...
  list(
    ...
    options = c("Frequent urination, Excessive thirst, Unexplained weight loss", "Tremor, Rigidity, Bradykinesia", 
                "Itching, Muscle cramps, Memory complaints", "Behavioral problems, Sleep issues, Depression"),
    correct = 1,
  ...
  list(
    ...
    options = c("Depression", "Postural instability", "Hypertension (high blood pressure)", "Memory complaints"),
    correct = 3,
  ...
  list(
    ...
    options = c("Alzheimer's and Diabetes", "Alzheimer's and Parkinson's", "Parkinson's and Kidney Disease", "Diabetes and Kidney Disease"),
    correct = 2,
  ...
  list(
    ...
    options = c("Alzheimer's only", "Parkinson's only", "Both Parkinson's and Kidney Disease (as comorbidity)", "Neither - diabetes only affects blood sugar"),
    correct = 3,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$navbar, ignoreInit = TRUE, {
    ...
    disease_tabs <- c("Alzheimer's" = "alz_info", "Kidney Disease" = "ckd_info", 
                      "Parkinson's" = "pk_info", "Diabetes" = "db_info")
    if (input$navbar %in% names(disease_tabs)) {
      updateNavbarPage(session, "navbar", selected = disease_tabs[[input$navbar]])
    }
  })
  
  # Info page "Learn More" buttons
  ...
  # "Assess Your Risk" buttons from info pages
  ...
  observeEvent(input$calc_alz, {
    output$alz_result_ui <- renderUI({
      new_data <- data.frame(
        FunctionalAssessment = input$alz_functional, ADL = input$alz_adl,
        MemoryComplaints = as.numeric(input$alz_memory), MMSE = input$alz_mmse,
        BehavioralProblems = as.numeric(input$alz_behavioral), EducationLevel = as.numeric(input$alz_edu)
      )
      prob <- predict(alz_model, newdata = new_data, type = "response")
      
      # 🔥 NEW: Render heat map
      output$alz_heatmap <- renderPlot({
        make_heatmap(alz_model, new_data, alz, var_labels)
      })
  ...
  observeEvent(input$calc_ckd, {
    output$ckd_result_ui <- renderUI({
      new_data <- data.frame(
        SerumCreatinine = input$ckd_creatinine, GFR = input$ckd_gfr, Itching = input$ckd_itching,
        FastingBloodSugar = input$ckd_fasting_bg, MuscleCramps = input$ckd_cramps, BUNLevels = input$ckd_bun,
        ProteinInUrine = input$ckd_protein, SystolicBP = input$ckd_systolicbp, HbA1c = input$ckd_hba1c, BMI = input$ckd_bmi
      )
      prob <- predict(ckd_model, newdata = new_data, type = "response")
      
      # 🔥 NEW: Render heat map
      output$ckd_heatmap <- renderPlot({
        make_heatmap(ckd_model, new_data, ckd, var_labels)
      })
      
      tagList(
        result_card(prob, "Chronic Kidney Disease", "ckd_heatmap"),
  ...
  observeEvent(input$calc_pk, {
    output$pk_result_ui <- renderUI({
      new_data <- data.frame(
        UPDRS = input$pk_updrs, Tremor = as.numeric(input$pk_tremor), FunctionalAssessment = input$pk_functional,
        Rigidity = as.numeric(input$pk_rigidity), Bradykinesia = as.numeric(input$pk_brady), MoCA = input$pk_moca,
        PosturalInstability = as.numeric(input$pk_postural), Age = input$pk_age, Depression = as.numeric(input$pk_depression),
        Diabetes = as.numeric(input$pk_diabetes), SleepDisorders = as.numeric(input$pk_sleepdisorders),
        TraumaticBrainInjury = as.numeric(input$pk_traumaticbraininjury)
      )
      prob <- predict(pk_model, newdata = new_data, type = "response")
      
      # 🔥 NEW: Render heat map
      output$pk_heatmap <- renderPlot({
        make_heatmap(pk_model, new_data, pk, var_labels)
      })
      
      tagList(
        result_card(prob, "Parkinson's Disease", "pk_heatmap"),
  ...
  observeEvent(input$calc_db, {
    output$db_result_ui <- renderUI({
      new_data <- data.frame(
        FastingBloodSugar = input$db_fasting_bg, HbA1c = input$db_hba1c,
        FrequentUrination = as.numeric(input$db_freq_urine), Hypertension = as.numeric(input$db_hypertension),
        ExcessiveThirst = as.numeric(input$db_thirst), UnexplainedWeightLoss = as.numeric(input$db_weight_loss),
        Smoking = as.numeric(input$db_smoking)
      )
      prob <- predict(db_model, newdata = new_data, type = "response")
      
      # 🔥 NEW: Render heat map
      output$db_heatmap <- renderPlot({
        make_heatmap(db_model, new_data, db, var_labels)
      })
      
      tagList(
        result_card(prob, "Type 2 Diabetes", "db_heatmap"),
  ...
  quiz_state <- reactiveValues(started = FALSE, current_q = 1, score = 0, answered = FALSE, 
                               selected = NULL, completed = FALSE, answers = rep(NA, length(quiz_questions)))
  ...
  observeEvent(input$start_quiz, {
    ...
    quiz_state$answers <- rep(NA, length(quiz_questions))
  })
  ...
  output$quiz_intro_ui <- renderUI({
    if (!quiz_state$started && !quiz_state$completed) {
      div(style = "background: white; border-radius: 10px; padding: 40px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.08);",
          tags$h1("🎯", style = "font-size: 64px; margin: 0;"),
          h3("Ready to Test Your Knowledge?", style = "color: #2C7BB6; margin-top: 10px;"),
          p("Answer questions about the risk factors used in our disease calculators.", style = "color: #666; font-size: 15px;"),
          tags$ul(style = "text-align: left; display: inline-block; color: #555; font-size: 14px;",
                  tags$li("Questions cover all four diseases"),
                  tags$li("Immediate feedback with detailed explanations"),
                  tags$li("Track your score and earn badges!")
          ),
          br(), br(),
  ...
  output$quiz_question_ui <- renderUI({
    if (quiz_state$started && !quiz_state$completed) {
      ...
      div(style = "background: white; border-radius: 10px; padding: 30px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);",
          ...
          option_buttons,
          if (quiz_state$answered) {
            div(class = "explanation-box", tags$b("💡 Did you know? "), q$explanation)
          },
          if (quiz_state$answered) {
            div(style = "margin-top: 20px; text-align: right;",
                actionButton("next_question", 
    ...
    if (quiz_state$started && !quiz_state$completed) {
      ...
      div(style = "margin-top: 20px;",
          ...
          div(class = "progress", style = "height: 8px; border-radius: 4px;",
              div(class = "progress-bar bg-info", role = "progressbar", style = sprintf("width: %s%%;", progress_pct))
          )
      )
    }
  })
  ...
  output$quiz_results_ui <- renderUI({
    if (quiz_state$completed) {
      ...
      div(style = "background: white; border-radius: 10px; padding: 40px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.08);",
          ...
          div(style = sprintf("display: inline-block; background: %s; color: white; padding: 8px 20px; border-radius: 20px; font-weight: 600; margin: 15px 0;", badge_color),
              badge_text),
          h1(paste0(quiz_state$score, "/", length(quiz_questions)), style = "color: #333; font-weight: 800; font-size: 48px; margin: 10px 0;"),
          div(class = "progress", style = "height: 30px; margin: 20px auto; max-width: 400px; border-radius: 15px;",
